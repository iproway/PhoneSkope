...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"Only %d item please!", nil), self.maximumImagesCount];
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"You can only select %d item at a time.", nil), self.maximumImagesCount];
...
- (void)selectedAssets:(NSArray *)assets
{
	...
	for(ALAsset *asset in assets) {
        ...
        //process chose resource is video
        if (obj && [obj isEqualToString:ALAssetTypeVideo]) {
            [workingDictionary setObject:[assetRep url] forKey:UIImagePickerControllerMediaURL];
        }
